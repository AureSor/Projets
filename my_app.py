...
def stock_prices():
    ...
    unique_tickers = df_filtered['TICKER'].unique().tolist() #on prend les ticker qui apparaisse dans le dataframe ci dessus
...
def load_data ():
    created_dataframe = session.create_dataframe(
        [[50, 25, "Q1"], [20, 35, "Q2"], [10, 30, "Q3"]],
        schema=["NAME", "FIST_BUMPS", "QUARTER"],
    )
    df = session.table("PC_DATAIKU_DB.RAW.JOB_POSTINGS")
    queried_data = created_dataframe.to_pandas()
    user_init = "test"
    return  user_init,df,queried_data

user_init, df,queried_data = load_data()

def search_tool(df=df,page_size=20):
    df = df.to_pandas()
    pagination = np.arange( 1,len(df)//page_size)
    index_review = st.selectbox('Please select page', pagination)
    df['page'] = np.arange(0,len(df))//20 +1
    st.dataframe(df[df['page']==index_review])
    st.write(f"Page : {index_review} ")
    user_immp_temp = st.selectbox(label="choix dans l'index", options =df[df['page']==index_review])
    return user_immp_temp

def select_list(df):
    n =st.selectbox( "choix du filtre", [column for column in df])
    st.write(f" vous avez choisit {n}")
    user_imp_temp= st.selectbox ("test", df[n],index=None, placeholder = f"Choisir un {n}")
    return user_imp_temp


def imp ():
    #user_imp à récupérer 
    user_imp = "test2"
    st.markdown(f"Vous êtes connecté au compte de {user_imp}")
